...
function fn(e) {
	...
	function T() {
		...
		var w = "IPDome";
		var Z = "IPZoom";
		var k = "<?xml version='1.0' encoding='utf-8'?><FileVersion>" + "<Platform name='win32'>" + "<npWebVideoKitPlugin.dll>3,0,6,1</npWebVideoKitPlugin.dll>" + "<WebVideoKitActiveX.ocx>3,0,6,1</WebVideoKitActiveX.ocx>" + "<PlayCtrl.dll>7,3,0,81</PlayCtrl.dll>" + "<StreamTransClient.dll>1,1,3,5</StreamTransClient.dll>" + "<SystemTransform.dll>2,5,1,7</SystemTransform.dll>" + "<NetStream.dll>1,0,5,59</NetStream.dll>" + "</Platform>" + "</FileVersion>";
		...
		e.PluginEventHandler = function (e, t, r) {
			W("插件事件：PluginEventHandler iEventType：%s iParam1: %s, iParam2: %s", e, t, r);
			if (M == e || q == e) {
				I.I_Stop(t)
			} else if (R == e) {
				I.I_StopRecord(t)
			...
			if (!h.browser().msie) {
				...
				for (var a = 0; a < r; a++) {
					if (navigator.mimeTypes[a].type.toLowerCase() == i.szMimeTypes) {
						e = "<embed align='center' type='" + i.szMimeTypes + "' width='" + t + "' height='" + s + "' name='" + o + "' wndtype='" + i.iWndowType + "' playmode='" + i.iPlayMode + "' colors='" + i.szColorProperty + "'>"
					}
				}
			} else {
				e = "<object classid='" + i.szOcxClassId + "' codebase='' standby='Waiting...' " + "id='" + n + "' width='" + t + "' height='" + s + "' align='center' >" + "<param name='wndtype' value='" + i.iWndowType + "'>" + "<param name='playmode' value='" + i.iPlayMode + "'>" + "<param name='colors' value='" + i.szColorProperty + "'></object>"
			}
			return e
		};
		var N = function () {
		...
		var F = function (e) {
			...
			I.I_GetAnalogChannelInfo(e.szIP, {
				success: function (t) {
					e.iAnalogChannelNum = r.$XML(t).find("VideoInputChannel", true).length
				}
			});
		...
		var O = function (e) {
			...
			e.oProtocolInc.getPortInfo(e, {
				async: false, success: function (e) {
					...
					for (var i = 0, a = o.length; i < a; i++) {
						if (r.$XML(o).eq(i).find("protocol").eq(0).text().toLowerCase() === "rtsp") {
							t = parseInt(r.$XML(o).eq(i).find("portNo").eq(0).text(), 10)
						}
						if (r.$XML(o).eq(i).find("protocol").eq(0).text().toLowerCase() === "http") {
							s = parseInt(r.$XML(o).eq(i).find("portNo").eq(0).text(), 10)
						}
						if (r.$XML(o).eq(i).find("protocol").eq(0).text().toLowerCase() === "dev_manage") {
							n = parseInt(r.$XML(o).eq(i).find("portNo").eq(0).text(), 10)
						}
					}
				}, error: function () {
					t = -1;
			...
			return {iRtspPort: t, iHttpPort: s, iDevicePort: n}
		};
		var j = function (e) {
			var t = -1, s = -1, n = -1;
			e.oProtocolInc.getUPnPPortStatus(e, {
				async: false, success: function (e) {
					...
					for (var i = 0, a = o.length; i < a; i++) {
						if (r.$XML(o).eq(i).find("internalPort").eq(0).text().toLowerCase() == "rtsp") {
							t = parseInt(r.$XML(o).eq(i).find("externalPort").eq(0).text(), 10)
						}
						if (r.$XML(o).eq(i).find("internalPort").eq(0).text().toLowerCase() == "http") {
							s = parseInt(r.$XML(o).eq(i).find("externalPort").eq(0).text(), 10)
						}
						if (r.$XML(o).eq(i).find("internalPort").eq(0).text().toLowerCase() == "admin") {
							n = parseInt(r.$XML(o).eq(i).find("externalPort").eq(0).text(), 10)
						}
					}
				}, error: function () {
					t = -1;
		...
		var K = function (e) {
			...
			e.oProtocolInc.getNetworkBond(e, {
				async: false, success: function (s) {
						...
						e.oProtocolInc.getNetworkInterface(e, {
							async: false, success: function (e) {
								...
								for (var n = 0, o = s.length; n < o; n++) {
									t.push({
										ipv4: r.$XML(e).find("ipAddress").eq(0).text(),
										ipv6: r.$XML(e).find("ipv6Address").eq(0).text()
									});
									break
								}
							}, error: function () {
		...
		var Q = function (e) {
			...
			if (e.oProtocolInc instanceof ot) {
				if (H == e.szDeviceType || w == e.szDeviceType || Z == e.szDeviceType) {
					e.oProtocolInc.getStreamChannels(e, {
						async: false, success: function (t) {
							e.oStreamCapa.bObtained = true;
							var r = $(t).find("streamingTransport", true).length;
							for (var s = 0; s < r; s++) {
								if ($(t).find("streamingTransport").eq(s).text().toLowerCase() == "shttp") {
									e.oStreamCapa.bObtained = true;
					...
					e.oProtocolInc.getSDKCapa(e, {
						async: false, success: function (t) {
							e.oStreamCapa.bObtained = true;
							e.oStreamCapa.bSupportShttpPlay = r.$XML(t).find("isSupportHttpPlay").eq(0).text() === "true";
							e.oStreamCapa.bSupportShttpPlayback = r.$XML(t).find("isSupportHttpPlayback").eq(0).text() === "true";
							e.oStreamCapa.bSupportShttpsPlay = r.$XML(t).find("isSupportHttpsPlay").eq(0).text() === "true";
							e.oStreamCapa.bSupportShttpsPlayback = r.$XML(t).find("isSupportHttpsPlayback").eq(0).text() === "true";
							e.oStreamCapa.bSupportShttpPlaybackTransCode = r.$XML(t).find("isSupportHttpTransCodePlayback").eq(0).text() === "true";
							e.oStreamCapa.bSupportShttpsPlaybackTransCode = r.$XML(t).find("isSupportHttpsTransCodePlayback").eq(0).text() === "true";
							if (r.$XML(t).find("ipChanBase", true).length > 0) {
								e.oStreamCapa.iIpChanBase = parseInt(r.$XML(t).find("ipChanBase").eq(0).text(), 10)
							}
						}, error: function () {
							e.oStreamCapa.bObtained = true
						}
		...
		var et = function (e) {
			...
			h.extend(t, e);
			if (t.TransFrameRate == "" || t.TransResolution == "" || t.TransBitrate == "") {
				return ""
			}
			var r = [];
			...
			r.push("<TransResolution>" + t.TransResolution + "</TransResolution>");
		...
		this.I_InsertOBJECTPlugin = function (t) {
			...
			if (document.getElementById(n) != null || document.getElementsByName(n).length != 0) {
				return -1
			}
		...
		this.I_GetLocalCfg = function () {
			...
			t.push("<ProtocolType>" + r.$XML(v).find("ProtocolType").eq(0).text() + "</ProtocolType>");
			t.push("<PackgeSize>" + r.$XML(v).find("PackgeSize").eq(0).text() + "</PackgeSize>");
			t.push("<PlayWndType>" + r.$XML(v).find("PlayWndType").eq(0).text() + "</PlayWndType>");
			t.push("<BuffNumberType>" + r.$XML(v).find("BuffNumberType").eq(0).text() + "</BuffNumberType>");
			t.push("<RecordPath>" + r.$XML(v).find("RecordPath").eq(0).text() + "</RecordPath>");
			t.push("<CapturePath>" + r.$XML(v).find("CapturePath").eq(0).text() + "</CapturePath>");
			t.push("<PlaybackFilePath>" + r.$XML(v).find("PlaybackFilePath").eq(0).text() + "</PlaybackFilePath>");
			t.push("<PlaybackPicPath>" + r.$XML(v).find("PlaybackPicPath").eq(0).text() + "</PlaybackPicPath>");
			t.push("<DownloadPath>" + r.$XML(v).find("DownloadPath").eq(0).text() + "</DownloadPath>");
			t.push("<IVSMode>" + r.$XML(v).find("IVSMode").eq(0).text() + "</IVSMode>");
			t.push("<CaptureFileFormat>" + r.$XML(v).find("CaptureFileFormat").eq(0).text() + "</CaptureFileFormat>");
			t.push("</LocalConfigInfo>");
			return h.loadXML(t.join(""))
		};
		this.I_SetLocalCfg = function (e) {
			var t = h.loadXML(e), s = -1;
			r.$XML(v).find("ProtocolType").eq(0).text(r.$XML(t).find("ProtocolType").eq(0).text());
			r.$XML(v).find("PackgeSize").eq(0).text(r.$XML(t).find("PackgeSize").eq(0).text());
			r.$XML(v).find("PlayWndType").eq(0).text(r.$XML(t).find("PlayWndType").eq(0).text());
			r.$XML(v).find("BuffNumberType").eq(0).text(r.$XML(t).find("BuffNumberType").eq(0).text());
			r.$XML(v).find("RecordPath").eq(0).text(r.$XML(t).find("RecordPath").eq(0).text());
			r.$XML(v).find("CapturePath").eq(0).text(r.$XML(t).find("CapturePath").eq(0).text());
			r.$XML(v).find("PlaybackFilePath").eq(0).text(r.$XML(t).find("PlaybackFilePath").eq(0).text());
			r.$XML(v).find("PlaybackPicPath").eq(0).text(r.$XML(t).find("PlaybackPicPath").eq(0).text());
			r.$XML(v).find("DownloadPath").eq(0).text(r.$XML(t).find("DownloadPath").eq(0).text());
			r.$XML(v).find("IVSMode").eq(0).text(r.$XML(t).find("IVSMode").eq(0).text());
			r.$XML(v).find("CaptureFileFormat").eq(0).text(r.$XML(t).find("CaptureFileFormat").eq(0).text());
		...
		this.I_StartRealPlay = function (e, t) {
			...
			var P = {iWndIndex: u, iStreamType: 1, iChannelID: 1, bZeroChannel: false};
			h.extend(P, t);
			if (s != -1) {
				Q(c[s]);
				var I = c[s];
				var m = parseInt(r.$XML(v).find("ProtocolType").eq(0).text(), 10);
				if (m == G && I.oStreamCapa.bSupportShttpPlay) {
					...
					if (P.iChannelID <= I.iAnalogChannelNum) {
						...
						p = I.oStreamCapa.iIpChanBase + parseInt(P.iChannelID, 10) - I.iAnalogChannelNum - 1
					}
					f = true;
					if (!E(P.iPort)) {
						I.iHttpPort = P.iPort;
				...
				h.extend(P, {urlProtocol: i, cgi: o, iPort: a, iStreamType: d, iChannelID: p});
		...
		this.I_PTZControl = function (e, t, r) {
			var s = {iWndIndex: u, iPTZIndex: e, iPTZSpeed: 4};
			h.extend(s, r);
			h.extend(s, {async: false});
		...
		this.I_RecordSearch = function (e, t, r, s, n) {
			...
			if (o != -1) {
				var i = c[o];
				var a = {iChannelID: t, szStartTime: r, szEndTime: s, iSearchPos: 0, success: null, error: null};
				h.extend(a, n);
		...
		this.I_StartPlayback = function (e, t) {
			...
			var d = {iWndIndex: u, iChannelID: 1, szStartTime: p + " 00:00:00", szEndTime: p + " 23:59:59"};
			h.extend(d, t);
			if (s != -1) {
				Q(c[s]);
				var f = c[s];
				var P = parseInt(r.$XML(v).find("ProtocolType").eq(0).text(), 10);
				if (P == G && f.oStreamCapa.bSupportShttpPlay) {
					...
					if (d.iChannelID <= f.iAnalogChannelNum) {
						...
						l = f.oStreamCapa.iIpChanBase + parseInt(d.iChannelID, 10) - f.iAnalogChannelNum - 1
					}
					if (!E(d.iPort)) {
						f.iHttpPort = d.iPort;
						a = d.iPort
				...
				h.extend(d, {urlProtocol: i, cgi: o, iPort: a, iChannelID: l});
				s = this.findWndIndexByIndex(d.iWndIndex);
				if (-1 == s) {
					d.szStartTime = d.szStartTime.replace(/[-:]/g, "").replace(" ", "T") + "Z";
		...
		this.I_StartDownloadRecord = function (e, t, r) {
			...
			if (s != -1) {
				var o = c[s];
				var i = {szPlaybackURI: t, szFileName: r};
		...
		this.I_CheckPluginInstall = function () {
			...
			if (t.msie) {
				try {
					var r = new ActiveXObject("WebVideoKitActiveX.WebVideoKitActiveXCtrl.1");
					e = 0
				} catch (s) {
				}
			} else {
		...
		this.I_SendHTTPRequest = function (e, t, r) {
			...
			var a = {type: "GET", url: i, auth: n.szAuth, success: null, error: null};
			h.extend(a, r);
			h.extend(a, {
				success: function (e) {
					if (r.success) {
		...
		this.I_RemoteConfig = function (e, t) {
			...
			if (r != -1) {
				...
				var p = "<RemoteInfo><DeviceInfo><DeviceType>" + o.iType + "</DeviceType>" + "<LanType>" + o.iLan + "</LanType>" + "<IP>" + e + "</IP>" + "<Port>" + n + "</Port>" + "<UName>" + u + "</UName>" + "<PWD>" + h.Base64.encode(l) + "</PWD></DeviceInfo></RemoteInfo>";
		...
		nt.prototype.submitRequest = function () {
			...
			if (this.options.async) {
				var r = a.HWP_SubmitHttpRequest(e, this.options.url, this.options.auth, this.options.data, this.options.timeout);
				if (r != -1) {
					t = {iRequestID: r, funSuccessCallback: this.options.success, funErrorCallback: this.options.error};
					this.m_httpRequestSet.push(t)
				}
			} else {
				var s = a.HWP_SendHttpSynRequest(e, this.options.url, this.options.auth, this.options.data, this.options.timeout);
				t = {funSuccessCallback: this.options.success, funErrorCallback: this.options.error};
				this.httpDataAnalyse(t, s)
			}
		};
		nt.prototype.getHttpMethod = function (e) {
			var t = {GET: 1, POST: 2, PUT: 5, DELETE: 6}, r = t[e];
			return r ? r : -1
		};
		...
		ot.prototype.login = function (e, t, s, n) {
			...
			var a = new nt;
			var u = {type: "GET", url: i, auth: s, success: null, error: null};
			h.extend(u, n);
			h.extend(u, {
				success: function (e) {
					if ("200" == r.$XML(e).find("statusValue").eq(0).text() || "OK" == r.$XML(e).find("statusString").eq(0).text()) {
						if (n.success) {
							n.success(e)
						}
					} else {
		...
		ot.prototype.getAudioInfo = function (e, t) {
			var r = _(this.CGI.getAudioInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
				success: function (e) {
					if (t.success) {
		...
		ot.prototype.getDeviceInfo = function (e, t) {
			var s = _(this.CGI.getDeviceInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var n = new nt;
			var o = {type: "GET", url: s, auth: e.szAuth, success: null, error: null};
			h.extend(o, t);
			h.extend(o, {
				success: function (e) {
					var s = [];
					s.push("<DeviceInfo>");
					s.push("<deviceName>" + h.escape(r.$XML(e).find("deviceName").eq(0).text()) + "</deviceName>");
					s.push("<deviceID>" + r.$XML(e).find("deviceID").eq(0).text() + "</deviceID>");
					s.push("<deviceType>" + r.$XML(e).find("deviceType").eq(0).text() + "</deviceType>");
					s.push("<model>" + r.$XML(e).find("model").eq(0).text() + "</model>");
					s.push("<serialNumber>" + r.$XML(e).find("serialNumber").eq(0).text() + "</serialNumber>");
					s.push("<macAddress>" + r.$XML(e).find("macAddress").eq(0).text() + "</macAddress>");
					s.push("<firmwareVersion>" + r.$XML(e).find("firmwareVersion").eq(0).text() + "</firmwareVersion>");
					s.push("<firmwareReleasedDate>" + r.$XML(e).find("firmwareReleasedDate").eq(0).text() + "</firmwareReleasedDate>");
					s.push("<encoderVersion>" + r.$XML(e).find("encoderVersion").eq(0).text() + "</encoderVersion>");
					s.push("<encoderReleasedDate>" + r.$XML(e).find("encoderReleasedDate").eq(0).text() + "</encoderReleasedDate>");
					s.push("</DeviceInfo>");
					e = h.loadXML(s.join(""));
					if (t.success) {
						t.success(e)
		...
		ot.prototype.getAnalogChannelInfo = function (e, t) {
			var s = _(this.CGI.getAnalogChannelInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var n = new nt;
			var o = {type: "GET", url: s, auth: e.szAuth, success: null, error: null};
			h.extend(o, t);
			h.extend(o, {
				success: function (e) {
					...
					for (var o = 0, i = n.length; o < i; o++) {
						...
						s.push("<id>" + r.$XML(a).find("id").eq(0).text() + "</id>");
						s.push("<inputPort>" + r.$XML(a).find("inputPort").eq(0).text() + "</inputPort>");
						s.push("<name>" + h.escape(r.$XML(a).find("name").eq(0).text()) + "</name>");
						s.push("<videoFormat>" + r.$XML(a).find("videoFormat").eq(0).text() + "</videoFormat>");
						s.push("</VideoInputChannel>")
					}
					s.push("</VideoInputChannelList>");
					e = h.loadXML(s.join(""));
		...
		ot.prototype.getDigitalChannel = function (e, t) {
			var s = _(this.CGI.getDigitalChannel, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var n = new nt;
			var o = {type: "GET", url: s, auth: e.szAuth, success: null, error: null};
			h.extend(o, t);
			h.extend(o, {
		...
		ot.prototype.getDigitalChannelInfo = function (e, t) {
			...
			this.getDigitalChannel(e, {
				async: false, success: function (e) {
					...
					for (var o = 0, i = t.length; o < i; o++) {
						var a = t[o], u = r.$XML(a).find("id").eq(0).text(), c = r.$XML(a).find("name").eq(0).text();
						n[u] = c
					}
				}, error: function (e, r) {
					if (t.error) {
			...
			var o = _(this.CGI.getDigitalChannelInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var i = new nt;
			var a = {type: "GET", url: o, auth: e.szAuth, success: null, error: null};
			h.extend(a, t);
			h.extend(a, {
				success: function (e) {
					var s = [];
					...
					for (var i = 0, a = o.length; i < a; i++) {
						...
						s.push("<proxyProtocol>" + r.$XML(u).find("proxyProtocol").eq(0).text() + "</proxyProtocol>");
						s.push("<addressingFormatType>" + r.$XML(u).find("addressingFormatType").eq(0).text() + "</addressingFormatType>");
						s.push("<ipAddress>" + r.$XML(u).find("ipAddress").eq(0).text() + "</ipAddress>");
						s.push("<managePortNo>" + r.$XML(u).find("managePortNo").eq(0).text() + "</managePortNo>");
						s.push("<srcInputPort>" + r.$XML(u).find("srcInputPort").eq(0).text() + "</srcInputPort>");
						s.push("<userName>" + h.escape(r.$XML(u).find("userName").eq(0).text()) + "</userName>");
						s.push("<streamType>" + r.$XML(u).find("streamType").eq(0).text() + "</streamType>");
						s.push("<online>" + r.$XML(u).find("online").eq(0).text() + "</online>");
		...
		ot.prototype.getZeroChannelInfo = function (e, t) {
			var r = _(this.CGI.getZeroChannelInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.getStreamChannels = function (e, t) {
			if (e.iAnalogChannelNum != 0) {
				var r = _(this.CGI.getStreamChannels.analog, e.szHttpProtocol, e.szIP, e.iCGIPort)
			} else {
				var r = _(this.CGI.getStreamChannels.digital, e.szHttpProtocol, e.szIP, e.iCGIPort)
			}
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
		...
		ot.prototype.getPPPoEStatus = function (e, t) {
			var r = _(this.CGI.getPPPoEStatus, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.getUPnPPortStatus = function (e, t) {
			var r = _(this.CGI.getUPnPPortStatus, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.getNetworkBond = function (e, t) {
			var r = _(this.CGI.getNetworkBond, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.getNetworkInterface = function (e, t) {
			var r = _(this.CGI.getNetworkInterface, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.getPortInfo = function (e, t) {
			var r = _(this.CGI.getPortInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		ot.prototype.startVoiceTalk = function (e, t) {
			var r = _(this.CGI.startVoiceTalk.open, e.szHttpProtocol, e.szIP, e.iCGIPort, t),
				s = _(this.CGI.startVoiceTalk.close, e.szHttpProtocol, e.szIP, e.iCGIPort, t),
				n = _(this.CGI.startVoiceTalk.audioData, e.szHttpProtocol, e.szIP, e.iCGIPort, t);
		...
		ot.prototype.ptzAutoControl = function (e, t, r, s) {
			...
			if (e.szDeviceType != w) {
				if (n <= e.iAnalogChannelNum) {
					o = _(this.CGI.ptzAutoControl.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
				} else {
					if (r.bShttpIPChannel) {
						o = _(this.CGI.ptzAutoControl.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum)
					} else {
						o = _(this.CGI.ptzAutoControl.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
					}
				}
				i = "<?xml version='1.0' encoding='UTF-8'?>" + "<autoPanData>" + "<autoPan>" + s.iPTZSpeed + "</autoPan>" + "</autoPanData>"
			} else {
				var a = 99;
				if (t) {
					a = 96
				}
				o = _(this.CGI.ptzAutoControl.ipdome, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID, a)
			}
			var u = new nt;
			var c = {type: "PUT", url: o, async: false, auth: e.szAuth, data: i, success: null, error: null};
			var l = this;
			h.extend(c, s);
			h.extend(c, {
				success: function (e) {
					...
					if (H == e.szDeviceType || Z == e.szDeviceType) {
						if (r.bShttpIPChannel) {
							o = _(l.CGI.ptzControl.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum)
						} else {
							o = _(l.CGI.ptzControl.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
						}
						i = "<?xml version='1.0' encoding='UTF-8'?>" + "<PTZData>" + "<pan>" + s.iPTZSpeed + "</pan>" + "<tilt>" + 0 + "</tilt>" + "</PTZData>";
						var a = new nt;
						var u = {
							type: "PUT",
							url: o,
		...
		ot.prototype.ptzControl = function (e, t, r, s) {
			...
			if (r.bPTZAuto) {
				this.ptzAutoControl(e, true, r, {iPTZSpeed: 0})
			}
			if (t) {
				s.iPTZSpeed = 0
			} else {
			...
			var i = [{}, {pan: 0, tilt: s.iPTZSpeed}, {pan: 0, tilt: -s.iPTZSpeed}, {
				pan: -s.iPTZSpeed,
				tilt: 0
			}, {pan: s.iPTZSpeed, tilt: 0}, {pan: -s.iPTZSpeed, tilt: s.iPTZSpeed}, {
				pan: -s.iPTZSpeed,
				tilt: -s.iPTZSpeed
			}, {pan: s.iPTZSpeed, tilt: s.iPTZSpeed}, {
				pan: s.iPTZSpeed,
				tilt: -s.iPTZSpeed
			}, {}, {speed: s.iPTZSpeed}, {speed: -s.iPTZSpeed}, {speed: s.iPTZSpeed}, {speed: -s.iPTZSpeed}, {speed: s.iPTZSpeed}, {speed: -s.iPTZSpeed}];
			var a = "";
			var u = {};
			switch (s.iPTZIndex) {
				case 1:
				...
				case 8:
					...
					a = "<?xml version='1.0' encoding='UTF-8'?>" + "<PTZData>" + "<pan>" + i[s.iPTZIndex].pan + "</pan>" + "<tilt>" + i[s.iPTZIndex].tilt + "</tilt>" + "</PTZData>";
					break;
				case 10:
				case 11:
					u = this.CGI.ptzControl;
					a = "<?xml version='1.0' encoding='UTF-8'?>" + "<PTZData>" + "<zoom>" + i[s.iPTZIndex].speed + "</zoom>" + "</PTZData>";
					break;
				case 12:
				case 13:
					u = this.CGI.ptzFocus;
					a = "<?xml version='1.0' encoding='UTF-8'?>" + "<FocusData>" + "<focus>" + i[s.iPTZIndex].speed + "</focus>" + "</FocusData>";
					break;
				case 14:
				case 15:
					u = this.CGI.ptzIris;
					a = "<?xml version='1.0' encoding='UTF-8'?>" + "<IrisData>" + "<iris>" + i[s.iPTZIndex].speed + "</iris>" + "</IrisData>";
					break;
				default:
					if (E(s.error)) {
						s.error()
			...
			if ((u == this.CGI.ptzFocus || u == this.CGI.ptzIris) && (e.szDeviceType == H || e.szDeviceType == w || e.szDeviceType == Z)) {
				o = _(u.ipc, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
			} else {
				if (n <= e.iAnalogChannelNum) {
					o = _(u.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
				} else {
					if (r.bShttpIPChannel) {
						o = _(u.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum)
					} else {
						o = _(u.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID)
					}
				}
			}
			var c = new nt;
			var l = {type: "PUT", url: o, async: false, auth: e.szAuth, data: a, success: null, error: null};
			h.extend(l, s);
			h.extend(l, {
				success: function (e) {
					if (s.success) {
		...
		ot.prototype.setPreset = function (e, t, r) {
			...
			if (s <= e.iAnalogChannelNum) {
				n = _(this.CGI.setPreset.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID)
			} else {
				if (t.bShttpIPChannel) {
					n = _(this.CGI.setPreset.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum, r.iPresetID)
				} else {
					n = _(this.CGI.setPreset.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID)
				}
			}
			...
			o += "</PTZPreset>";
			var i = new nt;
			var a = {type: "PUT", url: n, auth: e.szAuth, data: o, success: null, error: null};
			h.extend(a, r);
			h.extend(a, {
				success: function (e) {
					if (r.success) {
		...
		ot.prototype.goPreset = function (e, t, r) {
			...
			if (s <= e.iAnalogChannelNum) {
				n = _(this.CGI.goPreset.analog, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID)
			} else {
				if (t.bShttpIPChannel) {
					n = _(this.CGI.goPreset.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum, r.iPresetID)
				} else {
					n = _(this.CGI.goPreset.digital, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID)
				}
			}
			var o = new nt;
			var i = {type: "PUT", url: n, auth: e.szAuth, success: null, error: null};
			h.extend(i, r);
			h.extend(i, {
				success: function (e) {
					if (r.success) {
		...
		ot.prototype.recordSearch = function (e, t) {
			var s = "", n = "", o = t.iChannelID, i = t.szStartTime.replace(" ", "T") + "Z",
				a = t.szEndTime.replace(" ", "T") + "Z";
			s = _(this.CGI.recordSearch, e.szHttpProtocol, e.szIP, e.iCGIPort);
			n = "<?xml version='1.0' encoding='UTF-8'?>" + "<CMSearchDescription>" + "<searchID>" + new ct + "</searchID>" + "<trackList><trackID>" + (o * 100 + 1) + "</trackID></trackList>" + "<timeSpanList>" + "<timeSpan>" + "<startTime>" + i + "</startTime>" + "<endTime>" + a + "</endTime>" + "</timeSpan>" + "</timeSpanList>" + "<maxResults>40</maxResults>" + "<searchResultPostion>" + t.iSearchPos + "</searchResultPostion>" + "<metadataList>" + "<metadataDescriptor>//metadata.ISAPI.org/VideoMotion</metadataDescriptor>" + "</metadataList>" + "</CMSearchDescription>";
			var u = new nt;
			var c = {type: "POST", url: s, auth: e.szAuth, data: n, success: null, error: null};
			h.extend(c, t);
			h.extend(c, {
				success: function (e) {
					var s = [];
					s.push("<CMSearchResult>");
					s.push("<responseStatus>" + r.$XML(e).find("responseStatus").eq(0).text() + "</responseStatus>");
					s.push("<responseStatusStrg>" + r.$XML(e).find("responseStatusStrg").eq(0).text() + "</responseStatusStrg>");
					s.push("<numOfMatches>" + r.$XML(e).find("numOfMatches").eq(0).text() + "</numOfMatches>");
					s.push("<matchList>");
					var n = r.$XML(e).find("searchMatchItem", true);
					for (var o = 0, i = n.length; o < i; o++) {
						var a = n[o];
						s.push("<searchMatchItem>");
						s.push("<trackID>" + r.$XML(a).find("trackID").eq(0).text() + "</trackID>");
						s.push("<startTime>" + r.$XML(a).find("startTime").eq(0).text() + "</startTime>");
						s.push("<endTime>" + r.$XML(a).find("endTime").eq(0).text() + "</endTime>");
						s.push("<playbackURI>" + h.escape(r.$XML(a).find("playbackURI").eq(0).text()) + "</playbackURI>");
						s.push("<metadataDescriptor>" + r.$XML(a).find("metadataDescriptor").eq(0).text().split("/")[1] + "</metadataDescriptor>");
						s.push("</searchMatchItem>")
					}
					s.push("</matchList>");
					s.push("</CMSearchResult>");
		...
		ot.prototype.startDownloadRecord = function (e, t) {
			var r = _(this.CGI.startDownloadRecord, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = "<?xml version='1.0' encoding='UTF-8'?>" + "<downloadRequest>" + "<playbackURI> " + h.escape(t.szPlaybackURI) + "</playbackURI>" + "</downloadRequest>";
			return a.HWP_StartDownload(r, e.szAuth, t.szFileName, s)
		};
		ot.prototype.exportDeviceConfig = function (e) {
			var t = _(this.CGI.deviceConfig, e.szHttpProtocol, e.szIP, e.iCGIPort);
			return a.HWP_ExportDeviceConfig(t, e.szAuth, "", 0)
		};
		ot.prototype.importDeviceConfig = function (e, t) {
			var r = _(this.CGI.deviceConfig, e.szHttpProtocol, e.szIP, e.iCGIPort);
		...
		ot.prototype.restart = function (e, t) {
			...
			var s = new nt;
			var n = {type: "PUT", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
				success: function (e) {
					if (t.success) {
		...
		ot.prototype.restore = function (e, t, r) {
			...
			var n = new nt;
			var o = {type: "PUT", url: s, auth: e.szAuth, success: null, error: null};
			h.extend(o, r);
			h.extend(o, {
				success: function (e) {
					if (r.success) {
		...
		ot.prototype.startUpgrade = function (e, t) {
			var r = _(this.CGI.startUpgrade.upgrade, e.szHttpProtocol, e.szIP, e.iCGIPort),
				s = _(this.CGI.startUpgrade.status, e.szHttpProtocol, e.szIP, e.iCGIPort);
		...
		ot.prototype.set3DZoom = function (e, t, s, n) {
			...
			if (o <= e.iAnalogChannelNum) {
				i = _(this.CGI.set3DZoom, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID)
			} else {
				if (t.bShttpIPChannel) {
					i = _(this.CGI.set3DZoom, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID - e.oStreamCapa.iIpChanBase + 1 + e.iAnalogChannelNum)
				} else {
					i = _(this.CGI.set3DZoom, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID)
				}
			}
			var a = h.loadXML(s), u = parseInt(r.$XML(a).find("StartPoint").eq(0).find("positionX").eq(0).text(), 10),
				c = parseInt(r.$XML(a).find("StartPoint").eq(0).find("positionY").eq(0).text(), 10),
				l = parseInt(r.$XML(a).find("EndPoint").eq(0).find("positionX").eq(0).text(), 10),
				p = parseInt(r.$XML(a).find("EndPoint").eq(0).find("positionY").eq(0).text(), 10);
			var d = "<?xml version='1.0' encoding='UTF-8'?>" + "<position3D>" + "<StartPoint>" + "<positionX>" + u + "</positionX>" + "<positionY>" + (255 - c) + "</positionY>" + "</StartPoint>" + "<EndPoint>" + "<positionX>" + l + "</positionX>" + "<positionY>" + (255 - p) + "</positionY>" + "</EndPoint>" + "</position3D>";
			var f = new nt;
			var P = {type: "PUT", url: i, data: d, auth: e.szAuth, success: null, error: null};
			h.extend(P, n);
			h.extend(P, {
				success: function (e) {
					if (n.success) {
		...
		ot.prototype.getSDKCapa = function (e, t) {
			var r = _(this.CGI.SDKCapabilities, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		it.prototype.getDeviceInfo = function (e, t) {
			...
			h.extend(o, {
				success: function (e) {
					...
					s.push("<deviceName>" + h.escape(r.$XML(e).find("deviceName").eq(0).text()) + "</deviceName>");
					s.push("<deviceID>" + r.$XML(e).find("deviceID").eq(0).text() + "</deviceID>");
					s.push("<deviceType>" + r.$XML(e).find("deviceDescription").eq(0).text() + "</deviceType>");
					s.push("<model>" + r.$XML(e).find("model").eq(0).text() + "</model>");
					s.push("<serialNumber>" + r.$XML(e).find("serialNumber").eq(0).text() + "</serialNumber>");
					s.push("<macAddress>" + r.$XML(e).find("macAddress").eq(0).text() + "</macAddress>");
					s.push("<firmwareVersion>" + r.$XML(e).find("firmwareVersion").eq(0).text() + "</firmwareVersion>");
					s.push("<firmwareReleasedDate>" + r.$XML(e).find("firmwareReleasedDate").eq(0).text() + "</firmwareReleasedDate>");
					s.push("<encoderVersion>" + r.$XML(e).find("logicVersion").eq(0).text() + "</encoderVersion>");
					s.push("<encoderReleasedDate>" + r.$XML(e).find("logicReleasedDate").eq(0).text() + "</encoderReleasedDate>");
					s.push("</DeviceInfo>");
					e = h.loadXML(s.join(""));
					if (t.success) {
						t.success(e)
			...
			h.extend(a, {
				success: function (e) {
					...
					for (var i = 0, a = o.length; i < a; i++) {
						...
						s.push("<id>" + c + "</id>");
						s.push("<sourceInputPortDescriptor>");
						s.push("<proxyProtocol>" + r.$XML(u).find("adminProtocol").eq(0).text() + "</proxyProtocol>");
						s.push("<addressingFormatType>" + r.$XML(u).find("addressingFormatType").eq(0).text() + "</addressingFormatType>");
						s.push("<ipAddress>" + r.$XML(u).find("ipAddress").eq(0).text() + "</ipAddress>");
						s.push("<managePortNo>" + r.$XML(u).find("adminPortNo").eq(0).text() + "</managePortNo>");
						s.push("<srcInputPort>" + r.$XML(u).find("srcInputPort").eq(0).text() + "</srcInputPort>");
						s.push("<userName>" + h.escape(r.$XML(u).find("userName").eq(0).text()) + "</userName>");
						s.push("<streamType>" + r.$XML(u).find("streamType").eq(0).text() + "</streamType>");
						s.push("<online>" + r.$XML(u).find("online").eq(0).text() + "</online>");
		...
		it.prototype.getPortInfo = function (e, t) {
			var s = _(this.CGI.getPortInfo, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var n = new nt;
			var o = {type: "GET", url: s, auth: e.szAuth, success: null, error: null};
			h.extend(o, t);
			h.extend(o, {
				success: function (s) {
					...
					var o = r.$XML(s).find("AdminAccessProtocol", true);
					for (var i = 0, a = o.length; i < a; i++) {
						var u = o[i];
						n.push("<AdminAccessProtocol>");
						n.push("<id>" + r.$XML(s).find("id").eq(0).text() + "</id>");
						n.push("<enabled>" + r.$XML(s).find("enabled").eq(0).text() + "</enabled>");
						n.push("<protocol>" + r.$XML(s).find("protocol").eq(0).text().toUpperCase() + "</protocol>");
						n.push("<portNo>" + r.$XML(s).find("portNo").eq(0).text() + "</portNo>");
						n.push("</AdminAccessProtocol>")
					}
					P.getStreamChannels(e, {
						async: false, success: function (s) {
							if (r.$XML(s).find("rtspPortNo", true).length > 0) {
								var o = parseInt(r.$XML(s).find("rtspPortNo").eq(0).text(), 10);
								n.push("<AdminAccessProtocol>");
								n.push("<id>" + 4 + "</id>");
								...
								n.push("</AdminAccessProtocol>");
								n.push("</AdminAccessProtocolList>");
								var i = h.loadXML(n.join(""));
								if (t.success) {
									t.success(i)
								}
							} else {
								P.getStreamDynChannels(e, {
									async: false, success: function (e) {
										if (r.$XML(e).find("rtspPortNo", true).length > 0) {
											var s = parseInt(r.$XML(e).find("rtspPortNo").eq(0).text(), 10);
											n.push("<AdminAccessProtocol>");
											n.push("<id>" + 4 + "</id>");
											n.push("<enabled>" + "true" + "</enabled>");
											n.push("<protocol>" + "RTSP" + "</protocol>");
											n.push("<portNo>" + s + "</portNo>");
											n.push("</AdminAccessProtocol>");
											n.push("</AdminAccessProtocolList>");
											var o = h.loadXML(n.join(""));
											if (t.success) {
												t.success(o)
											}
										}
					...
					P.getStreamChannels(e, {
						async: false, success: function (n) {
							if (r.$XML(n).find("rtspPortNo", true).length > 0) {
								var o = parseInt(r.$XML(n).find("rtspPortNo").eq(0).text(), 10);
								...
								P.getStreamDynChannels(e, {
									async: false, success: function (e) {
										if (r.$XML(e).find("rtspPortNo", true).length > 0) {
											var n = parseInt(r.$XML(e).find("rtspPortNo").eq(0).text(), 10);
											s.push("<AdminAccessProtocol>");
											s.push("<id>" + 4 + "</id>");
											s.push("<enabled>" + "true" + "</enabled>");
											s.push("<protocol>" + "RTSP" + "</protocol>");
											s.push("<portNo>" + n + "</portNo>");
											s.push("</AdminAccessProtocol>");
											s.push("</AdminAccessProtocolList>");
											var o = h.loadXML(s.join(""));
											if (t.success) {
												t.success(o)
											}
										}
		...
		it.prototype.getStreamDynChannels = function (e, t) {
			var r = _(this.CGI.getStreamDynChannels, e.szHttpProtocol, e.szIP, e.iCGIPort);
			var s = new nt;
			var n = {type: "GET", url: r, auth: e.szAuth, success: null, error: null};
			h.extend(n, t);
			h.extend(n, {
		...
		it.prototype.ptzAutoControl = function (e, t, r, s) {
			...
			if (e.szDeviceType != w) {
				o = _(this.CGI.ptzAutoControl, e.szHttpProtocol, e.szIP, e.iCGIPort, n);
				i = "<?xml version='1.0' encoding='UTF-8'?>" + "<PTZData>" + "<pan>" + s.iPTZSpeed + "</pan>" + "<tilt>0</tilt>" + "</PTZData>"
			} else {
				var a = 99;
				if (t) {
				...
				o = _(this.CGI.goPreset, e.szHttpProtocol, e.szIP, e.iCGIPort, n, a)
			}
			var u = new nt;
			var c = {type: "PUT", url: o, async: false, auth: e.szAuth, data: i, success: null, error: null};
			var l = this;
			...
			h.extend(c, {
				success: function (e) {
					...
					if (e.szDeviceType != w) {
						o = _(l.CGI.ptzControl, e.szHttpProtocol, e.szIP, e.iCGIPort, r.iChannelID);
						var a = new nt;
						var u = {
							type: "PUT",
							url: o,
		...
		it.prototype.setPreset = function (e, t, r) {
			var s = t.iChannelID, n = "", o = "";
			n = _(this.CGI.setPreset, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID);
			o = "<?xml version='1.0' encoding='UTF-8'?>";
			o += "<PTZPreset>";
			o += "<id>" + r.iPresetID + "</id>";
			if (e.szDeviceType != w) {
		...
		it.prototype.goPreset = function (e, t, r) {
			var s = t.iChannelID, n = "";
			n = _(this.CGI.goPreset, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID, r.iPresetID);
			var o = new nt;
			var i = {type: "PUT", url: n, auth: e.szAuth, success: null, error: null};
			h.extend(i, r);
			h.extend(i, {
		...
		it.prototype.set3DZoom = function (e, t, r, s) {
			var n = _(this.CGI.set3DZoom, e.szHttpProtocol, e.szIP, e.iCGIPort, t.iChannelID);
			var o = new nt;
			var i = {type: "PUT", url: n, data: r, auth: e.szAuth, success: null, error: null};
			h.extend(i, s);
			h.extend(i, {
				success: function (e) {
					if (s.success) {
		...
		ut.prototype.browser = function () {
			...
			var a = e.exec(i) || t.exec(i) || r.exec(i) || s.exec(i) || n.exec(i) || i.indexOf("compatible") < 0 && o.exec(i) || ["unknow", "0"];
		...
		ut.prototype.dateFormat = function (e, t) {
			...
			for (var s in r) {
				if (new RegExp("(" + s + ")").test(t)) {
					t = t.replace(RegExp.$1, RegExp.$1.length == 1 ? r[s] : ("00" + r[s]).substr(("" + r[s]).length))
				}
			}
			return t
		};
		ut.prototype.Base64 = {
			...
			encode: function (e) {
				...
				while (c < e.length) {
					...
					t = t + this._keyStr.charAt(o) + this._keyStr.charAt(i) + this._keyStr.charAt(a) + this._keyStr.charAt(u)
				}
				return t
			},
			decode: function (e) {
			...
			_utf8_decode: function (e) {
				...
				while (r < e.length) {
					...
					if (s < 128) {
						...
						c2 = e.charCodeAt(r + 1);
						c3 = e.charCodeAt(r + 2);
						t += String.fromCharCode((s & 15) << 12 | (c2 & 63) << 6 | c3 & 63);
						r += 3
					}
				}
				return t
		...
		ct.prototype.createUUID = function () {
			...
			var c = ct.getIntegerBits(ct.rand(8191), 0, 7) + ct.getIntegerBits(ct.rand(8191), 8, 15) + ct.getIntegerBits(ct.rand(8191), 0, 7) + ct.getIntegerBits(ct.rand(8191), 8, 15) + ct.getIntegerBits(ct.rand(8191), 0, 15);
		...
		o = "webVideoCtrl" + lt;
		if (typeof e.attachEvent != "object" && h.browser().msie) {
			var pt = "<script for=" + n + " event='GetSelectWndInfo(SelectWndInfo)'>GetSelectWndInfo(SelectWndInfo);</script>";
			pt += "<script for=" + n + " event='ZoomInfoCallback(szZoomInfo)'>ZoomInfoCallback(szZoomInfo);</script>";
			pt += "<script for=" + n + "  event='GetHttpInfo(lID, lpInfo, lReverse)'>GetHttpInfo(lID, lpInfo, lReverse);</script>";
			pt += "<script for=" + n + "  event='PluginEventHandler(iEventType, iParam1, iParam2)'>PluginEventHandler(iEventType, iParam1, iParam2);</script>";
			document.write(pt)
		}
		return this
	};
	var r = e.WebVideoCtrl = new T();
	r.version = "1.0.9"
};
export default fn